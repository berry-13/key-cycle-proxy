use key_cycle_proxy::{
    ...
    proxy::{KeyPool, ProxyEngine, UpstreamClient, ProxyError},
...
fn test_config_loading_from_environment() {
    ...
    assert!(result.is_ok(), "Config loading should succeed with env var: {:?}", result.err());
...
fn test_proxy_error_status_codes() {
    assert_eq!(ProxyError::NoKeyAvailable { model: "test".to_string() }.status_code(), axum::http::StatusCode::INTERNAL_SERVER_ERROR);
    assert_eq!(ProxyError::MethodNotAllowed.status_code(), axum::http::StatusCode::METHOD_NOT_ALLOWED);
    assert_eq!(ProxyError::Timeout.status_code(), axum::http::StatusCode::GATEWAY_TIMEOUT);
    assert_eq!(ProxyError::RateLimited.status_code(), axum::http::StatusCode::TOO_MANY_REQUESTS);
    assert_eq!(ProxyError::PayloadTooLarge.status_code(), axum::http::StatusCode::PAYLOAD_TOO_LARGE);
}
...
async fn test_latency_measurement_timeout() {
    ...
    assert!(duration < Duration::from_secs(10), "Latency measurement should timeout quickly");
}