...
async fn test_load_performance_100_concurrent_requests() {
    ...
    assert!(
        ...
        "At least 95% of requests should succeed, got {}/{}",
        successful_requests,
        num_requests
    );
    ...
    assert!(
        throughput > 20.0,
        "Throughput should be > 20 req/s, got {:.2}",
        throughput
    );
    assert!(
        avg_request_time < Duration::from_millis(500),
        "Average request time should be < 500ms, got {:?}",
        avg_request_time
    );
}
...
async fn test_memory_usage_under_load() {
    ...
    println!(
        "Completed {} batches of {} requests each",
...
async fn test_error_resilience_under_load() {
    ...
    println!(
        "Resilience test: {} successful, {} failed",
        successful, failed
    );

    // Even with some servers failing, we should still have high success rate due to key rotation
    assert!(
        successful >= num_requests * 80 / 100,
        "At least 80% requests should succeed due to key rotation, got {}/{}",
        successful,
        num_requests
    );
}
